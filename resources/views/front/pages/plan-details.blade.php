...
@section('content')
    ...
    <script>
                                ...
                                Promise.all(promises).then(() => {
                                    ...
                                    html2pdf()
                                        ...
                                        .then(pdf => {
                                            ...
                                            // Right-side text
                                            const dateText =
                                                `Nutrition Training Plan | ${new Date().toLocaleDateString('en-GB')}`;
                                            const dateFontSize = 9; // smaller font
                                            const dateColor = "#649ef7"; // blue
                                            const dateX = pageWidth - 0.5;
                                            const dateY = circleCenterY + 0.04; // align with circle
                                            ...
                                            for (let i = 1; i <= totalPages; i++) {
                                                ...
                                                // Add logo (left, vertically centered)
                                                if (window.logoBase64) {
                                                    pdf.addImage(window.logoBase64, 'PNG', logoX, logoY,
                                ...
                                $('.clickable').on('click', function() {
                                    ...
                                    $.ajax({
                                        ...
                                        success: function(response) {
                                            ...
                                            // 🖼️ Set meal title and description
                                            $('#recipeDialogModal .modal-body .dialog-header h2')
                                                .text(meal.meal
                                                    .title || 'Meal');
                                            $('#recipeDialogModal .modal-body .dialog-header p')
                                                .text(meal.meal
                                                    .description || '');
                                            ...
                                            meal.user_items.forEach(function(userItem) {
                                                ...
                                                const selectedUnits = item
                                                    .selected_qty_unit || [];
                                                const selected = selectedUnits.find(u => u
                                                    .checked) || null;
                                            ...
                                            $('#recipeDialogModal .modal-body ul').html(
                                                ingredientsHtml);
                                            ...
                                            // 🔢 Nutrition Info
                                            $('#recipeDialogModal .modal-body .nutrition-info')
                                                .html(`
                                    ...
                                    success: function(data) {
                                        if (data.items && data.items.length > 0) {
                                            $.each(data.items, function(index, item) {
                                                ...
                                                try {
                                                    selectedUnits = typeof item.selected_qty_unit ===
                                                        'string' ?
                                                        JSON.parse(item.selected_qty_unit) :
                                                ...
                                                if (checkedUnits.length > 0) {
                                                    const formattedUnits = checkedUnits.map(u => {
                                                        let qtyText = u.qty?.toString()
                                                            .trim() || '';
                                                        const unitText = (u.unit || '').trim();
                                                        const needsSpace = !["g", "ml", "mL"]
                                                            .includes(unitText
                                                                .toLowerCase());

                                                        const numericQty = Number(qtyText);
                                                        if (!isNaN(numericQty)) {
                                                            qtyText = numericQty % 1 === 0 ?
        ...
        `);
                                ...
                                $.ajax({
                                    url: '{{ route('front.items.swap-items', ':id') }}'.replace(':id',
                                            itemId) +
                                    ...
                                    success: function(data) {
                                        ...
                                        const mainQtyText = formatQtyUnit(item.selected_qty_unit, item.qty,
                                            item.unit);
                                        let mainItem = `
                                                ...
                                                error: function(xhr, status, error) {
                                                        ...
                                                        if (xhr.status === 422) {
                                                            ...
                                                            $('body').on('click', '.apply-changes-btn', function() {
                                                                ...
                                                                const userMealId = $(this).data('user-meal-id');
                                                                const userPlanId = $(this).data('user-plan-id');
                                                                const userSubCategoryId = $(this).data(
                                                                    'user-sub-category-id');
                                                                const userCategoryId = $(this).data(
                                                                    'user-category-id');
                                                                ...
                                                                $.ajax({
                                                                    ...
                                                                    success: function(response) {
                                                                        ...
                                                                        if (response.success) {
                                                                            $('#smartSwapModal').modal(
                                                                                'hide');
                                                                            var meal_id = response.data[
                                                                                'meal_id'];
                                                                            var meal_name = response
                                                                                .data['meal_name'];
                                                                            var user_meal_id = response
                                                                                .data['user_meal_id'];
                                                                            mealItemModelReload(meal_id,
                                                                                meal_name,
                                                                                user_meal_id,
                                                                                userSubCategoryId,
                                                                                userPlanId,
                                                            ...
                                                            document.addEventListener('DOMContentLoaded', function() {
                                                                var inviteDiv = document.querySelector(
                                                                    '.share-dropdown-item');
                                                                if (inviteDiv) {
                                                                    inviteDiv.addEventListener('click', function(
                                                                        e) {
                                                                        e.preventDefault();
                                                                        var comingSoonModal = new bootstrap
                                                            ...
                                                            $(document).ready(function() {
                                                                ...
                                                                $('.slider-wrapper').each(function(idx) {
                                                                    var $wrapper = $(this);
                                                                    var $scroll = $wrapper.find(
                                                                        '.challenge-cards');
                                                                    var $cards = $scroll.find(
                                                                        '.challenge-card');
                                                                    var cardWidth = $cards.length ? $cards
                                                                        .outerWidth(true) : 200;

                                                                    $wrapper.find('.left-arrow').on('click',
                                                                        function(e) {
                                                                            e.preventDefault();
                                                                            var before = $scroll
                                                                                .scrollLeft();
