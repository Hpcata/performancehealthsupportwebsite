                                                    ...
                                                    @foreach ($userMeal->userItems as $userItem)
                                                        @php
                                                            $matchedItem = $userMeal->meal->userMealItems
                                                                ->filter(function ($item) use ($userItem, $userPlan) {
                                                                    return $item->id == $userItem->item_id
                                                                        && $item->pivot->user_id == $userPlan->user_id;
                                                                })
                                                                ->first();
                                    ...
                                    <tbody>
                                        @foreach ($userMealTime->userCategories as $userCategory)
                                            ...
                                            @foreach ($userCategory->userMeals as $userMeal)
                                                @if (in_array($userMeal->meal_id, $selectedMeals))
                                                    <tr>
                                                        <td>
                                                            <img src="{{ url('private/public/storage/'.$userMeal->meal->image ?? '') }}" alt="Meal image">
                                                        </td>
                                                        ...
                                                        <td>
                                                            <ul>
                                                                @foreach ($userMeal->userItems as $userItem)
                                                                    @php
                                                                        $matchedItem = $userMeal->meal->userMealItems
                                                                            ->filter(fn ($item) => $item->id == $userItem->item_id && $item->pivot->user_id == $userPlan->user_id)
                                                                            ->first();
