			...
			<div class="h-right d-flex align-items-center order-1">
				...
				<div class="dropdown user-profile ms-3 d-flex align-items-center zindex-popover">
					<div class="u-info me-2">
						<p class="mb-0 text-end line-height-sm "><span class="font-weight-bold">{{ Auth::guard('admin')->user()->name ?? '' }}</span></p>
						<small>{{ Auth::guard('admin')->user()->designation ?? '' }}</small>
					</div>
					<a class="nav-link dropdown-toggle pulse p-0" href="#" role="button" data-bs-toggle="dropdown" data-bs-display="static">
					<img class="avatar lg rounded-circle img-thumbnail"
					src="{{ isset(Auth::guard('admin')->user()->profile_image) ? asset('private/public/' . Auth::guard('admin')->user()->profile_image) : 'https://booking.biohealthpassport.com.au/public/admin/dist/assets/images/profile_av.svg' }}" 
					alt="profile">
					</a>
						...
						<div class="card border-0 w280">
							<div class="card-body pb-0">
								<div class="d-flex py-1">
									<img class="avatar rounded-circle" src="{{ isset(Auth::guard('admin')->user()->profile_image) ? asset('private/public/' . Auth::guard('admin')->user()->profile_image) : 'https://booking.biohealthpassport.com.au/public/admin/dist/assets/images/profile_av.svg' }}" alt="profile">
									<div class="flex-fill ms-3">
										<p class="mb-0"><span class="font-weight-bold">{{ Auth::guard('admin')->user()->name ?? '' }}</span></p>
										<small class="d-block text-truncate" style="max-width: 170px;" data-bs-toggle="tooltip" title="{{ Auth::guard('admin')->user()->email ?? '' }}">{{ Auth::guard('admin')->user()->email ?? '' }}</small>
									</div>
								</div>
							...
							<div class="list-group m-2 ">
								<a href="{!! route('backend.admin-profile', ['id' => Auth::guard('admin')->user()->id ?? 3 ]) !!}" class="list-group-item list-group-item-action border-0 "><i class="icofont-ui-file fs-5 me-3"></i>Profile Page</a>
