...
$(document).ready(function() {
   ...
   // Safely access the injected URL
    console.log(apiUrl);
    ...
    const testimonialApiUrl = window.AppConfig.testimonialsApiUrl; 
    $.ajax({
    ...
    const organizationsApiUrl = window.AppConfig.organizationsApiUrl; 
    $.ajax({
        url: organizationsApiUrl,
        method: 'GET',
        success: function(response) {
            console.log(response);
            // response is the organization object: { top: [...], bottom: [...] }
            if (response && response.top) {
                populateMarquee('marquee-top', response.top, 250, 0);
            }
            if (response && response.bottom) {
                populateMarquee('marquee-bottom', response.bottom, 0, 150);
            }
        },
        error: function(xhr, status, error) {
            console.error('Failed to fetch data from API:', error);
