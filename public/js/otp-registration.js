...
function validateCountryCode() {
    ...
    return selectedValue && selectedValue !== '' && 
           selectedValue !== 'undefined' && selectedValue.startsWith('+');
}
...
function sendOtp() {
    ...
    if (!mobileRegex.test(mobileInputValue)) {
        showError('Please enter a valid mobile number (7-15 digits)', 'mobile_number');
        return;
    }
...
function verifyOtp() {
    ...
    .then(data => {
        if (data.success) {
            ...
            if (data.errors && data.errors.otp) {
                ...
                showError('Invalid OTP. Please try again.', 'otp-input-group');
            }
    ...
    .catch(error => {
        console.error('Error:', error);
        showError('Network error. Please check your connection and try again.', 'otp-input-group');
    })
    .finally(() => {
        // Reset button
        button.textContent = originalText;
...
function clearFieldError(element) {
    ...
    if (fieldId) {
        const fieldErrors = document.querySelectorAll(`[data-field="${fieldId}"]`);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('click', function(e) {
        if (e.target.matches('#signupModal .btn')) {
            const generalErrors = document.querySelectorAll('.error-message:not([data-field])');
    ...
    $('#signupModal').on('shown.bs.modal', function () {
        ...
        if (countrySelect) {
            ...
            if (typeof countrySelect === 'string') {
                ...
                // If it's a DOM element (span), set text content
                if (!countrySelect.textContent || countrySelect.textContent.trim() === '') {
                ...
                const displayElement = countrySelect.parentNode.querySelector('.selected-code');
                if (displayElement) {
